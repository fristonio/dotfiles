...
eval grey='$FG[145]'
PROMPT='
%{$cyan%} [fr1st0n10] > %{$reset_color%}${_current_dir}$(git_prompt_info)
%{$cyan%}❮❮%{$reset_color%} '
...
RPROMPT='$(_vi_status)%{$(echotc UP 1)%}$(git_prompt_short_sha) ${_return_status} %{$grey%}%T%{$(echotc DO 1)%}%{$reset_color%}'
...
function _user_host() {
  echo "%{$red%}%n%{$reset_color%}%{$grey%} at %{$yellow%}%m%{$reset_color%} %{$grey%}in "
}
...
ZSH_THEME_GIT_PROMPT_PREFIX="%{$grey%}on %{$blue%}"
